...
class lightingNet(nn.Module):
    ...
    def forward(self, innerFeat, target_light, count, skip_count):
        x = innerFeat[:,0:self.ncInput,:,:].clone() # lighting feature
        _, _, row, col = x.shape

        # predict lighting
        feat = x.clone().mean(dim=(2,3), keepdim=True)
        ...
        innerFeat[:,0:self.ncInput,:,:] = upFeat.clone()
        return innerFeat, innerFeat[:, self.ncInput:, :, :].clone(), light
